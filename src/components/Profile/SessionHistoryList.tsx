...
const SessionHistoryList: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await fetch(`${CONFIG.BACKEND_URL}/api/sessions/${deleteId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${tokens.accessToken}`
        }
  ...
  const handleDownloadPDF = (session: Session) => {
    try {
      ...
      doc.text(`Session ID: ${session.id.toUpperCase()}`, 20, 37)
      ...
      doc.save(`scribe-session-${session.id.slice(-4)}.pdf`)
          ...
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: '1rem', width: '100%' }}>
              <Typography variant="bodyText" sx={{ fontWeight: '600', minWidth: '100px', opacity: 0.7 }}>
                {new Date(session.createdAt).toLocaleDateString(undefined, { month: 'short', day: '2-digit', year: 'numeric' })}
              </Typography>
              <Typography variant="bodyText" sx={{ fontWeight: '500', flex: 1 }}>
                Session {session.id.slice(-4).toUpperCase()}
              </Typography>
              <Chip 
                label={session.sourceLanguage.toUpperCase()} 
                size="small" 
